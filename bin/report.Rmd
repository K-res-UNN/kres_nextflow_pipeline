...
# VirulenceFinder output (for Enterococcus)
virulencefinder <- NULL
if (file.exists(params$virulencefinder) &&
    file.size(params$virulencefinder) > 0) {

    virulencefinder <- read.csv(params$virulencefinder_out, sep = "\t", header = TRUE)
}
...
db_df <- enframe(unlist(yml)) %>%
  ...
  rename(Version = value) %>%
  select(-module) %>%
  filter(Name == "amrfinderplus-database" | Name == "PlasmidFinder database" | Name == "VirulenceFinder database" | Name == "mlst database")
```
\vspace{1em}
...
\begin{enumerate}[nosep, leftmargin=*, itemsep=0pt, topsep=0pt, partopsep=0pt, parsep=0pt]
  ...
  \item \textbf{Genomlengde:} $\,\geq\,$ \textbf{95\%} og $\,\leq\,$ \textbf{105\%} av minste/største lukkede genom
...
b) Ribosomal MLST - Species og støtteprosent:
...
AMR_2nd_criteria <- AMR_out %>%
  filter(Method == "POINTX" | Method == "POINT_DISRUPT") %>%
  arrange(Class, Subclass) %>%
  select(Element, Class, Subclass, Coverage, Identity)
...
if (!is.null(lrefinder)) {
  ...
  cat("\\textbf{e) Enterokokk-spesifikk resistensanalyse:}\n")
  cat("\\vspace{1em}\n\\par\n")
  cat("\\textbf{Linezolidresistens:}\n")
  cat("\\vspace{1em}\n")
...
if (!is.null(virulencefinder)) {
    ...
    virulencefinder <- virulencefinder %>%
        select(virulence_gene, identity, coverage)

    virulence_tbl <- knitr::kable(
        virulencefinder,
        format = "latex",
        booktabs = TRUE,
        longtable = TRUE,
...
\vspace{0.5em}
Analysen gjelder bare Enterobacterales og Gram-positive
...
db_tbl <- knitr::kable(
  ...
  #footnote(general = "versjon for mlst database er satt til dato for nedlasting av mlst-skjema fra pubMLST og pasteurMLST", general_title = "") %>%
  kableExtra::kable_styling(
    latex_options = c("striped", "repeat_header"),
    full_width = FALSE,
    position = "left",
...
print(db_tbl)
cat("\n\\textsuperscript{*}")
cat("\\text{\\footnotesize   versjon for mlst database er satt til dato for nedlasting av mlst-skjema fra pubMLST og pasteurMLST.}")
```
